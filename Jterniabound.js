    ...
    a.atob ||
      (a.atob = function (a) {
        ...
        for (
          var d = 0, e, f, g = 0, h = "";
          (f = a.charAt(g++));
          ~f && ((e = d % 4 ? e * 64 + f : f), d++ % 4)
            ? (h += String.fromCharCode(255 & (e >> ((-2 * d) & 6))))
            : 0
        )
          f = b.indexOf(f);
        return h;
...
var Sburb = (function (Sburb) {
  ...
  Sburb.testCompatibility = function (div, levelName, includeDevTools) {
    ...
    if (
      !(
        "xhrblob" in Modernizr &&
        "xhrarraybuffer" in Modernizr &&
        "datauri" in Modernizr
      )
    ) {
      setTimeout(function () {
    ...
    if (!Modernizr.xmlserializer)
      errors.push("- Lack of XMLSerializer support.");

    if (errors.length) {
      // Display what failed
      var deploy =
        '<div style="padding-left: 0; padding-right: 0; margin-left: auto; margin-right: auto; display: block; width:650px; height:450px; overflow: auto;">';
      deploy +=
        '<p style="font-weight: bold;">Your browser is too old. Here are the problems we found:</p>';
      ...
      if (
        ...
        Modernizr.xhrarraybuffer &&
        Modernizr.arraybuffer &&
        Modernizr.arraybuffer.dataview &&
        Modernizr.blob &&
        Modernizr.blob.url &&
        Modernizr.blob.creator
      ) {
  ...
  Sburb.initialize = function (div, levelName, includeDevTools) {
    ...
    if (Sburb.crashed) return; // Hard crash if the browser is too old. testCompatibility() will handle the error message
  ...
  function update() {
    ...
    if (!Sburb.loadingRoom) Sburb.curRoom.update();
  ...
  var _onkeydown = function (e) {
    ...
    if (Sburb.updateLoop && !Sburb.inputDisabled) {
      ...
      if (Sburb.chooser.choosing) {
        ...
        if (
          e.keyCode == Sburb.Keys.space &&
          !Sburb.pressed[Sburb.Keys.space] &&
          Sburb.engineMode == "wander"
        ) {
          Sburb.chooser.choices = [];
          var queries = Sburb.char.getActionQueries();
          for (var i = 0; i < queries.length; i++) {
            Sburb.chooser.choices = Sburb.curRoom.queryActions(
              Sburb.char,
              queries[i].x,
              queries[i].y,
            );
    ...
    if (!Sburb.pressed[e.keyCode]) Sburb.pressedOrder.push(e.keyCode);
  ...
  var _onkeyup = function (e) {
    ...
    if (Sburb.pressed[e.keyCode]) Sburb.pressedOrder.destroy(e.keyCode);
  ...
  Sburb.onMouseUp = function (e, canvas) {
    ...
    if (
      Sburb.dialoger &&
      Sburb.dialoger.box &&
      Sburb.dialoger.box.isVisuallyUnder(Sburb.Mouse.x, Sburb.Mouse.y)
    ) {
      Sburb.dialoger.nudge();
    }
  };
  ...
  function focusCamera() {
    ...
    if (!Sburb.destFocus) {
      ...
      Math.abs(Sburb.destFocus.x - Sburb.cam.x - Sburb.Stage.width / 2) > 4 ||
      Math.abs(Sburb.destFocus.y - Sburb.cam.y - Sburb.Stage.height / 2) > 4
    ) {
      Sburb.cam.x +=
        (Sburb.destFocus.x - Sburb.Stage.width / 2 - Sburb.cam.x) / 5;
      Sburb.cam.y +=
        (Sburb.destFocus.y - Sburb.Stage.height / 2 - Sburb.cam.y) / 5;
    } else {
      Sburb.focus = Sburb.destFocus;
      Sburb.destFocus = null;
    }
    Sburb.Stage.x = Math.max(
      0,
      Math.min(
        Math.round(Sburb.cam.x / Sburb.Stage.scaleX) * Sburb.Stage.scaleX,
        Sburb.curRoom.width - Sburb.Stage.width,
      ),
    );
    Sburb.Stage.y = Math.max(
      0,
      Math.min(
        Math.round(Sburb.cam.y / Sburb.Stage.scaleX) * Sburb.Stage.scaleX,
        Sburb.curRoom.height - Sburb.Stage.height,
      ),
    );
  }
  ...
  function handleRoomChange() {
    if (Sburb.destRoom || Sburb.fading) {
      if (Sburb.Stage.fade < 1.1) {
        Sburb.Stage.fade = Math.min(
          1.1,
          Sburb.Stage.fade + Sburb.Stage.fadeRate,
        );
  ...
  function chainAction() {
    ...
    for (var i = 0; i < Sburb.actionQueues.length; i++) {
      ...
      if (queue.paused || queue.waitFor) {
        if (
          (queue.trigger && queue.trigger.checkCompletion()) ||
          queue.waitFor
        ) {
          queue.paused = false;
          queue.trigger = null;
  ...
  Sburb.performAction = function (action, queue) {
    ...
    if (
      ((Sburb.curAction &&
        Sburb.curAction.followUp != action &&
        Sburb.curAction != action) ||
        !hasControl()) &&
      action.soft
    ) {
      return null;
  ...
  function performActionInQueue(action, queue) {
    ...
    do {
      ...
      queue.curAction &&
      queue.curAction.times <= 0 &&
      queue.curAction.followUp &&
      queue.curAction.followUp.noDelay
    );
  }
  ...
  Sburb.changeBGM = function (newSong) {
    if (newSong) {
      if (Sburb.bgm) {
        if (
          Sburb.bgm.asset == newSong.asset &&
          Sburb.bgm.startLoop == newSong.startLoop
        ) {
