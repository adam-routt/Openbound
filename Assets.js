var Sburb = (function (Sburb) {
  ...
  Sburb.AssetManager.prototype.loop = function () {
    ...
    if (!Sburb.pressed[Sburb.Keys.space]) this.space = false;
  ...
  Sburb.AssetManager.prototype.draw = function () {
    ...
    if (Sburb.tests.loading == 0) {
      ...
      Sburb.stage.fillText(
        "Warning: File loading is unreliable. Use a newer browser, like Chrome.",
        Sburb.Stage.width / 2,
        Sburb.Stage.height - 35,
      );
    }
  ...
  Sburb.AssetManager.prototype.purge = function () {
    for (var k in this.recurrences) {
      if (this.recurrences.hasOwnProperty(k)) clearTimeout(this.recurrences[k]);
    }
    this.totalLoaded = 0;
    this.totalAssets = 0;
    this.totalMeta = 0;
  ...
  Sburb.AssetManager.prototype.loadAsset = function (assetObj) {
    ...
    var loadedAsset = this.assets[name].assetOnLoadFunction(function () {
      oThis.assetLoaded(name);
    });
    if (!loadedAsset)
      this.assets[name].assetOnFailFunction(function () {
        oThis.assetFailed(name);
      });
  };
  ...
  Sburb.base64ArrayBuffer = function (arrayBuffer) {
    ...
    var bytes = new window[Sburb.prefixed("Uint8Array", window, false)](
      arrayBuffer,
    );
  ...
  Sburb.loadGenericAsset = function (asset, path, id) {
    ...
    if (assetPath in Sburb.assetManager.blobs) {
      ...
      }, 0); // Async call success so things don't blow up
      return;
    }
    ...
    if ([4, 5, 6, 7, 8, 9, 10, 11].contains(Sburb.tests.loading)) {
        ...
        if ((this.status == 200 || this.status == 0) && this.response) {
          ...
          if ([5, 6, 7, 9, 10, 11].contains(Sburb.tests.loading)) {
            ...
            if (Sburb.tests.loading == 11) {
              ...
              var builder = new window[
                Sburb.prefixed("BlobBuilder", window, false)
              ]();
              builder.append(this.response);
              blob = builder.getBlob(type);
            } else if (Sburb.tests.loading == 9) {
              blob = this.response[
                Sburb.prefixed("slice", Blob.prototype, false)
              ](0, this.response.size, type);
          ...
          if ([2, 3].contains(Sburb.tests.loading)) {
            ...
            for (var i = 0; i < len; i += QUANTUM) {
              binstr += String.fromCharCode.apply(
                null,
                bytes.slice(i, Math.min(i + QUANTUM, len)),
              );
            }
      ...
      xhr.onload = function () {
        if ((this.status == 200 || this.status == 0) && this.responseText) {
          // Checking responseBody directly doesn't work??
          // Clean the string
      ...
      console.error(
        "Invalid Sburb.tests.loading. Value = " + Sburb.tests.loading,
      );
      asset.failure(id);
      cleanup();
    }
  ...
  Sburb.createMovieAsset = function (name, path) {
    ...
    ret.done = function (url) {
      ret.src = url;
      Sburb.Bins["movie"].innerHTML +=
        '<div id="' +
        name +
        '"><object classid="clsid:d27cdb6e-ae6d-11cf-96b8-444553540000" codebase="http://fpdownload.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version=9,0,0,0" id="movie" width="' +
        Sburb.Stage.width +
        '" height="' +
        Sburb.Stage.height +
        '"><param name="allowScriptAccess" value="always" /\><param name="wmode" value="transparent"/\><param name="movie" value="' +
        ret.src +
        ...
        '" height="' +
        Sburb.Stage.height +
        '" swLiveConnect="true" id="movie' +
        name +
        '" name="movie' +
        name +
        '" allowScriptAccess="always" type="application/x-shockwave-flash" pluginspage="http://www.macromedia.com/go/getflashplayer" /\></object></div>';
