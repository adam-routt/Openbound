var Sburb = (function (Sburb) {
  ...
  commands.randomTalk = function (info) {
    ...
    var randomNum = Math.floor(
      Math.random() * (Sburb.dialoger.queue.length + 1),
    );
    if (randomNum) {
  ...
  commands.teleport = function (info) {
    ...
    Sburb.playEffect(
      Sburb.effects["teleportEffect"],
      Sburb.char.x,
      Sburb.char.y,
    );
    var params = parseParams(info);
    Sburb.curAction.followUp = new Sburb.Action(
      "playEffect",
      "teleportEffect," + params[1] + "," + params[2],
      null,
      null,
  ...
  commands.playSong = function (info) {
    ...
    Sburb.changeBGM(
      new Sburb.BGM(Sburb.assets[params[0]], parseFloat(params[1])),
    );
  };
  ...
  commands.openChest = function (info) {
    ...
    var lastAction;
    var newAction = (lastAction = new Sburb.Action(
      "waitFor",
      "played," + chest.name,
      null,
      null,
    ...
    lastAction = lastAction.followUp = new Sburb.Action(
      "addSprite",
      item.name + "," + Sburb.curRoom.name,
      null,
      null,
      null,
      true,
  ...
  commands.disableControl = function (info) {
    Sburb.inputDisabled =
      info.trim().length > 0 ? new Sburb.Trigger(info) : true;
  };
  ...
  commands.pauseActionQueueGroup = commands.pauseActionQueueGroups = function (
    info,
  ) {
    var params = parseParams(info);
    for (var i = 0; i < params.length; i++) {
  ...
  //syntax: group name or list of group names
  commands.resumeActionQueueGroup = commands.resumeActionQueueGroups =
    function (info) {
      var params = parseParams(info);
      for (var i = 0; i < params.length; i++) {
        Sburb.forEachActionQueueInGroup(params[i], function (queue) {
  ...
  //syntax: group name or list of group names
  commands.cancelActionQueueGroup = commands.cancelActionQueueGroups =
    function (info) {
      var params = parseParams(info);
      for (var i = 0; i < params.length; i++) {
  ...
  commands.teleportRemote = function (info) {
    ...
    var params = parseParams(info);
    Sburb.curAction.followUp.followUp.followUp = new Sburb.Action(
      "playEffect",
      "teleportEffect," + params[2] + "," + params[3],
      null,
      null,
      Sburb.curAction.followUp.followUp.followUp,
    );
  };
  ...
  commands.save = function (info) {
    ...
    Sburb.saveStateToStorage(
      Sburb.char.name + ", " + Sburb.curRoom.name,
      auto,
      local,
    );
  };
    ...
    if (Sburb.isStateInStorage(false, local)) {
      actions.push(
        new Sburb.Action(
          "load",
          "false, " + local,
          "Load " + Sburb.getStateDescription(false),
        ),
      );
    }
    if (Sburb.isStateInStorage(true, local)) {
      actions.push(
        new Sburb.Action(
          "load",
          "true, " + local,
          "Load " + Sburb.getStateDescription(true),
        ),
      );
    }
