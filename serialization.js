var Sburb = (function (Sburb) {
  ...
  Sburb.serialize = function (sburbInst) {
    ...
    var output =
      ...
      (Sburb.bgm
        ? "' bgm='" +
          Sburb.bgm.asset.name +
          (Sburb.bgm.startLoop ? "," + Sburb.bgm.startLoop : "")
        : "") +
      (Sburb.Stage.scaleX != 1 ? "' scale='" + Sburb.Stage.scaleX : "") +
      (Sburb.assetManager.resourcePath
        ? "' resourcePath='" + Sburb.assetManager.resourcePath
        : "") +
  ...
  Sburb.loadStateFromStorage = function (auto, local) {
    ...
    for (var key in storage) {
      ...
      if (savedIndex >= 0) // this key is a saved state
      {
        if (
          auto &&
          key.indexOf("(auto)") >= 0 &&
          key.indexOf(Sburb.name + ":" + Sburb.version) >= savedIndex
        ) {
          saveStateName = key;
          break;
        } else if (
    ...
    var decoded = Iuppiter.decompress(
      Iuppiter.Base64.decode(Iuppiter.toByteArray(compressed), true),
    );
  ...
  Sburb.deleteOldVersionStates = function (local) {
    ...
    for (var key in storage) {
      ...
      if (savedIndex >= 0) // this key is a saved state
      {
        ...
        if (
          key.indexOf(Sburb.name + ":") >= savedIndex &&
          key.indexOf(":" + Sburb.version) < 0
        )
          storage.removeItem(key);
      }
    }
  ...
  function serializeGameState(output, gameState) {
    ...
    for (var key in gameState) {
      ...
      output = output.concat(
        "  <" + key + ">" + encodeXML(gameState[key]) + "</" + key + ">",
      );
    }
    output = output.concat("\n</gameState>\n");
      ...
      if (curAsset.type == "graphic") {
        ...
        for (var i = 0; i < curAsset.points.length; i++) {
          innerHTML = innerHTML.concat(
            curAsset.points[i].x + "," + curAsset.points[i].y,
          );
  ...
  function serializeTemplates(output, templates) {
    ...
    try {
      ...
      for (var template in templates) {
        ...
        output = output.concat(
          serializer.serializeToString(templates[template]),
        );
      }
    } catch (e) {
  ...
  function loadSerial(serialText, keepOld) {
    ...
    if (levelPath) {
      Sburb.assetManager.levelPath =
        levelPath.value.charAt(levelPath.value.length - 1) == "/"
          ? levelPath.value
          : levelPath.value + "/";
    }
  ...
  function parseSerialAsset(curAsset) {
    ...
    if (blobUrlsAttr) blobUrls = blobUrlsAttr.value.split(";");
  ...
  function parseTemplateClasses(input) {
    ...
    if (classes.length > 0) {
      ...
      for (var i = 0; i < templates.length; i++) {
        ...
        if (
          templateNode.nodeName != "#text" &&
          templateNode.nodeName != "#comment"
        ) {
  ...
  function parseState(input) {
    ...
    if (bgm) {
      ...
      Sburb.changeBGM(
        new Sburb.BGM(
          Sburb.assets[params[0]],
          parseFloat(params.length > 1 ? params[1] : "0"),
        ),
      );
    }
    ...
    if (rootInfo.getNamedItem("startAction")) {
      ...
      for (var i = 0; i < input.childNodes.length; i++) {
        ...
        if (
          tmp.tagName == "action" &&
          tmp.attributes.getNamedItem("name").value == initActionName
        ) {
  ...
  function serialLoadDialogSprites(dialogSprites, assetFolder) {
    ...
    if (!Sburb.dialoger.dialogSpriteLeft) {
      Sburb.dialoger.dialogSpriteLeft = new Sburb.Sprite(
        "dialogSprite",
        -1000,
        Sburb.Stage.height,
        0,
      ...
      Sburb.dialoger.dialogSpriteRight = new Sburb.Sprite(
        "dialogSprite",
        Sburb.Stage.width + 1000,
        Sburb.Stage.height,
        0,
        0,
      );
    ...
    for (var i = 0; i < animations.length; i++) {
      Sburb.dialoger.dialogSpriteLeft.addAnimation(
        Sburb.parseAnimation(animations[i], assetFolder),
      );
      Sburb.dialoger.dialogSpriteRight.addAnimation(
        Sburb.parseAnimation(animations[i], assetFolder),
      );
    }
  }
  ...
  function serialLoadRoomSprites(newRoom, roomSprites, spriteFolder) {
    for (var j = 0; j < roomSprites.length; j++) {
      ...
      var actualSprite =
        spriteFolder[curSprite.attributes.getNamedItem("name").value];
